...
- **Installer:** copy the built `PhantomFence.exe`, `README.md`, `PRIVACY.md`, and `src/phantomfence.ico` into `packaging/installer/`, then compile `PhantomFence.iss` with the free [Inno Setup 6.3+](https://jrsoftware.org/isinfo.php).
- **Microsoft Store (MSIX):** copy the built `PhantomFence.exe` into `packaging/store/`, then run `build-msix.ps1` on Windows (see `packaging/store/STORE-SUBMISSION.md`).
- The Store logo assets in `packaging/store/Assets/` are generated by `tools/make_assets.py` (Python + Pillow).
